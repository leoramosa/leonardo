...
function useSearchDatos(DatosCategory) {
  const [query, setQuery] = useState('');
  const [filteredResults, setfilteredResults] = useState(DatosCategory);

  React.useMemo(() => {
    const result = DatosCategory.filter((dates) => {
...
function Product(props) {
  ...
  const DatosCategory = props.DatosCategory;
  const { query, setQuery, filteredResults } = useSearchDatos(DatosCategory);
